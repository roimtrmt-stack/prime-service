...
const CACHE_NAME = "prime-service-cache-v10";
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.includes("/functions/v1/") || url.pathname.includes("/rest/v1/")) {
    // Rejet intercepté : sans catch, l'échec réseau remonte au service worker
    // comme « TypeError: Failed to fetch ». La réponse 503 conserve le comportement
    // attendu par la page (réponse non-ok gérée par le code applicatif).
    event.respondWith(
      fetch(event.request).catch(() => new Response(null, { status: 503, statusText: "Hors ligne" })),
    );
    return;
  }
  if (event.request.method !== "GET") {
    // Non-GET (POST, PUT...) : ne pas intercepter. Le navigateur gère nativement,
    // ce qui évite les TypeError « Failed to fetch » remontés par le service worker.
    return;
  }
  ...
  event.respondWith(
    caches.match(event.request).then((cached) => {
      ...
      return fetch(event.request)
        ...
        .catch(() =>
          // Hors ligne ou requête interrompue : on tente le cache une seconde fois
          // (utile si l'entrée vient d'être mise en cache), sinon réponse 503.
          caches.match(event.request).then(
            (match) => match || new Response(null, { status: 503, statusText: "Hors ligne" }),
          ),
        );
    }),
  );
