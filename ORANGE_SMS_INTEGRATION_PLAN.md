...
La nouvelle fonction `envoyer-sms-orange` est active en version 1 avec `verify_jwt=true`. Elle refuse les appels sans authentification administrateur, résout les numéros de commande depuis `commandes` côté serveur et les destinataires généraux depuis `produits` côté serveur. La fonction `clever-processor` reste active et son envoi web push natif VAPID existant a été conservé ; son CORS accepte désormais aussi GitHub Pages.

Le bouton général de l’espace vendeur appelle toujours `clever-processor` pour le push, puis ajoute un appel séparé à `envoyer-sms-orange` pour les boutiques enregistrées si Orange est activé. Le bouton manuel associé à une commande appelle désormais `notifier-commande-manuellement`, qui crée la même file `notifications_boutiquiers` que le checkout et déclenche le worker `notifier-boutiquier` pour le push immédiat, les rappels à 3/6/9 minutes, le bouton rouge et l’escalade Discord. L’ancien push direct web VAPID via `clever-processor` reste conservé comme repli. Orange ne participe pas à ce parcours manuel gratuit et reste désactivé tant que ses secrets ne sont pas configurés.

La configuration d’envoi Orange reste désactivée tant que les secrets Orange ne sont pas ajoutés. Elle n’est pas nécessaire pour les notifications push manuelles ou automatiques, qui restent gratuites côté site. Pour activer Orange plus tard, ajouter dans Supabase `ORANGE_CLIENT_ID`, `ORANGE_CLIENT_SECRET`, `ORANGE_SENDER_ADDRESS` validé par Orange, `ORANGE_SMS_ENABLED=true` et `SMS_PROVIDER=orange`. Pour une transition avec TextBee, utiliser `SMS_PROVIDER=both`. Ne jamais activer le canal avant d’avoir obtenu l’approbation Orange Mali, un bundle de crédits et un test HTTP 201 suivi d’une réception réelle.
