...
const tiktokSimulation = await read("simulation-tiktok-commande-electronique.html");
...
const falaServiceMigration = await read("supabase/migrations/202609030002_normalize_fala_service_name.sql");
const imageOptimizationMigration = await read("supabase/migrations/202608220010_automatic_product_image_optimization.sql");
const imageQueueLockMigration = await read("supabase/migrations/202608220011_lock_image_processing_queue.sql");
...
assert.match(orderFunction, /ORANGE_PAYMENT_NUMBER/);
assert.match(orderFunction, /PAIEMENT CLIENT À RECEVOIR/);
assert.match(orderFunction, /Composer #144#/);
assert.match(orderFunction, /Ne jamais communiquer ni enregistrer le code secret/);
...
assert.match(supabaseConfig, /embellir-photo-produit/);
assert.match(deployWorkflow, /functions deploy embellir-photo-produit/);
...
assert.match(index, /id="vue-boutiques"/);
assert.match(index, /function afficherBoutiques\(/);
assert.match(index, /function cleBoutiquePourProduit\(/);
assert.match(index, /function nomBoutiqueAffichable\(/);
assert.match(index, /\["service de fala", "sevice de fala", "fala service"\]/);
assert.match(index, /if\(nom === "Fala Service"\) return "Fala Service\\|"/);
...
assert.match(index, /afficherBoutiques\('\$\{categorie\.id\}', '\$\{sc\.id\}'\)/);
assert.match(index, /cleBoutiquePourProduit\(p\) === cleBoutique/);
assert.match(index, /vue: "boutiques"/);
...
assert.match(inscriptionPage, /traiterEtPublierArticle/);
assert.match(inscriptionPage, /analyserAspectsPhotoGratuite/);
...
assert.doesNotMatch(inscriptionPage, /normaliserNomBoutiquePourComparaison/);
...
assert.match(index, /photo-optimizer\.js\?v=natural-1/);
assert.match(index, /preparerImageUploadFidele/);
assert.match(inscriptionPage, /preparerImageUploadFidele/);
assert.match(inscriptionPage, /photo-optimizer\.js\?v=natural-1/);
assert.match(photoOptimizer, /preparerImageUploadFidele/);
assert.doesNotMatch(photoOptimizer, /fillRect|fillStyle|masque|fondSortie|imageFondBlanc/);
assert.match(inscriptionPage, /for\(const fichierOriginal of fichiersSelectionnes\)/);
assert.doesNotMatch(inscriptionPage, /compresserImageAncienne|chargerImagePourCorrection/);
assert.doesNotMatch(index + inscriptionPage, /photo-optimizer\.js\?v=white-1|fond blanc|fond catalogue uniforme/);
...
assert.doesNotMatch(inscriptionPage, /trouverArticleEquivalent|score >= 0\.90|fusionner_stock_si_meme_boutique|action: "fusion"/);
assert.match(inscriptionPage, /Chaque photo sélectionnée devient une ligne distincte/);
assert.match(inscriptionPage, /action: "creation"/);
...
assert.match(index, /DUREE_VERROUILLAGE_CLIENT_MERCI_MS = 5 \* 60 \* 1000 \+ 30 \* 1000/);
assert.match(index, /beforeunload/);
assert.match(index, /gererClavierPendantMerci/);
assert.match(index, /merci-client-verrouillee/);
assert.match(index, /btnRetourMerci/);
assert.match(index, /CLE_BOUTIQUES_PREFEREES/);
assert.match(index, /function basculerBoutiquePreferee\(/);
assert.match(index, /Mes boutiques préférées/);
assert.match(falaServiceMigration, /set nom_boutique = 'Fala Service'/);
assert.match(falaServiceMigration, /where lower\(regexp_replace/);
assert.match(index, /est-favori/);
assert.match(index, /confirmerRetourAccueilDepuisMerci/);
assert.match(index, /Êtes-vous sûr d’avoir envoyé l’argent au numéro/);
assert.doesNotMatch(index, /id="btnAppelClientMerci"/);
assert.doesNotMatch(index, /Appeler Prime Service hors connexion/);
assert.doesNotMatch(index, /href:\s*`tel:/);
assert.match(index, /numeroPaiementAffiche/);
assert.match(index, /NUMERO_PAIEMENT/);
...
assert.match(index, /permissionNavigateur === "granted"/);
assert.match(index, /Finalisez l’activation pour recevoir les nouveaux articles/);
assert.match(index, /abonnements_push/);
...
assert.match(imageOptimizationFunction, /@cross\/image@0\.4\.3/);
assert.doesNotMatch(imageOptimizationFunction, /detectContentBounds|borderBackground|buildStudioImage|fillRect|composite|brightness\(|contrast\(|saturation\(|sharpen\(/);
assert.match(imageOptimizationFunction, /applyExifOrientation/);
assert.match(imageOptimizationFunction, /resizeWithoutCropping/);
assert.match(imageOptimizationFunction, /OUTPUT_MAX_DIMENSION = 1200/);
assert.match(imageOptimizationFunction, /outputFormatFromBytes/);
assert.match(imageOptimizationFunction, /source remplacée/);
assert.doesNotMatch(imageOptimizationFunction, /OPENAI|GEMINI|GROQ|OPENROUTER|CLOUDFLARE/i);
...
assert.match(boutiquePage, /IMPORTANT POUR LE BOUTIQUIER ET LE LIVREUR/);
assert.match(boutiquePage, /paiement du client se fait par/);
assert.match(boutiquePage, /Orange Money/);
assert.match(boutiquePage, /Prévenez votre livreur/);
assert.match(boutiquePage, /Paiement[\s\S]*IMPORTANT POUR LE BOUTIQUIER ET LE LIVREUR/);
assert.doesNotMatch(boutiquePage, /avant la livraison/);
assert.doesNotMatch(boutiquePage, /94134408|commission.*Orange Money/);
...
assert.match(tiktokSimulation, /Paiement[\s\S]*IMPORTANT POUR LE BOUTIQUIER ET LE LIVREUR/);
assert.doesNotMatch(tiktokSimulation, /avant la livraison|Le client paie par/);
assert.match(boutiquePage, /expiresAt/);
assert.match(boutiquePage, /Quartier de livraison/);
assert.match(cleverProcessor, /web-push@3\.6\.7/);
assert.match(cleverProcessor, /VAPID_PUBLIC_KEY/);
assert.match(cleverProcessor, /VAPID_PRIVATE_KEY/);
assert.match(cleverProcessor, /webpush\.sendNotification/);
assert.match(cleverProcessor, /abonnements_push/);
assert.doesNotMatch(cleverProcessor, /onesignal/i);
assert.doesNotMatch(index, /onesignal/i);
assert.match(boutiquePage, /function addGoogleMapsLink/);
assert.match(boutiquePage, /function normaliserTelephoneMali/);
assert.match(boutiquePage, /function addCallLink/);
assert.match(boutiquePage, /href = `tel:\$\{normalized\}`/);
assert.match(boutiquePage, /Appeler le client hors connexion/);
...
assert.match(imageOptimizationMigration, /traitements_images_produits/);
assert.match(imageOptimizationMigration, /produits_queue_image_optimization/);
assert.match(imageOptimizationMigration, /after insert or update of image_url/);
assert.match(imageOptimizationMigration, /cron\.schedule/);
assert.match(imageOptimizationMigration, /embellir-photo-produit/);
assert.match(imageOptimizationMigration, /optimized/);
assert.match(imageQueueLockMigration, /Aucun acces public file images/);
assert.match(imageQueueLockMigration, /using \(false\)/);
assert.match(imageQueueLockMigration, /with check \(false\)/);
