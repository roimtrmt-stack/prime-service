...
const groqFunction = await read("supabase/functions/analyser-article-groq/index.ts");
...
const openRouterFunction = await read("supabase/functions/analyser-article-openrouter/index.ts");
const cloudflareFunction = await read("supabase/functions/analyser-article-cloudflare/index.ts");
assert.match(cloudflareFunction, /CLOUDFLARE_ACCOUNT_ID/);
assert.match(cloudflareFunction, /CLOUDFLARE_API_TOKEN/);
assert.match(cloudflareFunction, /api\.cloudflare\.com\/client\/v4\/accounts/);
assert.match(cloudflareFunction, /@cf\/llava-hf\/llava-1\.5-7b-hf/);
assert.match(cloudflareFunction, /image/);
assert.match(cloudflareFunction, /fetchImageAsBase64/);
assert.match(cloudflareFunction, /cloudflare_quota_or_rate_limit/);
assert.match(cloudflareFunction, /cloudflare_key_rejected/);
assert.match(cloudflareFunction, /cloudflare_invalid_output/);
assert.match(cloudflareFunction, /origin.*!== ALLOWED_ORIGIN|headers\.get\("origin"\).*!== ALLOWED_ORIGIN/);
assert.doesNotMatch(cloudflareFunction, /CLOUDFLARE_API_TOKEN\s*[:=]\s*["']/);
assert.match(openRouterFunction, /OPENROUTER_API_KEY/);
assert.match(openRouterFunction, /openrouter\.ai\/api\/v1\/chat\/completions/);
assert.match(openRouterFunction, /openrouter\/free/);
assert.match(openRouterFunction, /image_url/);
assert.match(openRouterFunction, /data:\$\{mimeType\};base64/);
assert.match(openRouterFunction, /openrouter_quota_or_rate_limit/);
assert.match(openRouterFunction, /openrouter_key_rejected/);
assert.match(openRouterFunction, /openrouter_invalid_output/);
assert.match(openRouterFunction, /origin.*!== ALLOWED_ORIGIN|headers\.get\("origin"\).*!== ALLOWED_ORIGIN/);
assert.doesNotMatch(openRouterFunction, /OPENROUTER_API_KEY\s*[:=]\s*["']/);
assert.match(groqFunction, /GROQ_API_KEY/);
assert.match(groqFunction, /api\.groq\.com\/openai\/v1\/chat\/completions/);
assert.match(groqFunction, /qwen\/qwen3\.6-27b/);
...
assert.match(groqFunction, /image_fetch_failed/);
assert.match(groqFunction, /image_type_not_allowed/);
assert.match(groqFunction, /origin.*!== ALLOWED_ORIGIN|headers\.get\("origin"\).*!== ALLOWED_ORIGIN/);
assert.doesNotMatch(groqFunction, /GROQ_API_KEY\s*[:=]\s*["']/);
...
assert.match(supabaseConfig, /\[functions\.analyser-article-groq\]/);
assert.match(supabaseConfig, /\[functions\.analyser-article-groq\][\s\S]*?verify_jwt = true/);
assert.match(supabaseConfig, /\[functions\.analyser-article-openrouter\]/);
assert.match(supabaseConfig, /\[functions\.analyser-article-openrouter\][\s\S]*?verify_jwt = true/);
assert.match(supabaseConfig, /\[functions\.analyser-article-cloudflare\]/);
assert.match(supabaseConfig, /\[functions\.analyser-article-cloudflare\][\s\S]*?verify_jwt = true/);
...
assert.match(inscriptionPage, /analyser-article-cloudflare/);
assert.match(inscriptionPage, /Cloudflare AI indisponible ou quota atteint/);
